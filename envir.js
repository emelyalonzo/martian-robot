...
export default function move(robotX, robotY, robotOrientation, robotLost, boundX, boundY, instructions, lostRobots, scent) {
  //To follow the instructions, I'll apply a for loop
  let orientation = robotOrientation;
  let x = robotX;
  let y = robotY;
  let position = [x, y, orientation];
  let isLost = robotLost;
  for (let i = 0; i < instructions.length; i++){
      const command = instructions[i]; //This takes the new instruction
      position[2] = command;
